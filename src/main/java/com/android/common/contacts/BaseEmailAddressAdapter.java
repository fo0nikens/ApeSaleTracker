...
public abstract class BaseEmailAddressAdapter extends CompositeCursorAdapter implements Filterable {
    ...
    private static class DirectoryListQuery {
        ...
        public static final String[] PROJECTION = new String[]{DIRECTORY_ID, DIRECTORY_ACCOUNT_NAME, DIRECTORY_ACCOUNT_TYPE, DIRECTORY_DISPLAY_NAME, DIRECTORY_PACKAGE_NAME, DIRECTORY_TYPE_RESOURCE_ID};
    ...
    protected int getItemViewType(int partitionIndex, int position) {
        return ((DirectoryPartition) getPartition(partitionIndex)).loading ? 1 : 0;
    }
    ...
    protected void onDirectoryLoadFinished(CharSequence constraint, Cursor directoryCursor, Cursor defaultPartitionCursor) {
        ...
        if (directoryCursor != null) {
            ...
            for (Partition partition2 : directories) {
        ...
        for (i = 1; i < count; i++) {
            ...
            if (partition.loading) {
                ...
                this.mHandler.sendMessageDelayed(this.mHandler.obtainMessage(1, i, 0, partition), 1000);
    ...
    private Cursor removeDuplicatesAndTruncate(int partition, Cursor cursor) {
        ...
        while (cursor.moveToNext() && count < 10) {
            String displayName = cursor.getString(0);
            if (!isDuplicate(cursor.getString(1), partition)) {
                newCursor.addRow(new Object[]{displayName, cursor.getString(1)});
                count++;
            }
        }
        cursor.close();
        ...
        while (partition < partitionCount) {
            if (!(partition == excludePartition || isLoading(partition))) {
                ...
                if (cursor != null) {
                    cursor.moveToPosition(-1);
                    while (cursor.moveToNext()) {
                        if (TextUtils.equals(emailAddress, cursor.getString(1))) {
                            return true;
                        }
                    }
                    continue;
