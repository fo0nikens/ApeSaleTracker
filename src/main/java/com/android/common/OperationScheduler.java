...
public class OperationScheduler {
    ...
    public long getNextTimeMillis(Options options) {
        ...
        long moratoriumTimeMillis = getTimeBefore("OperationScheduler_moratoriumTimeMillis", options.maxMoratoriumMillis + getTimeBefore("OperationScheduler_moratoriumSetTimeMillis", now));
    ...
    public String toString() {
        ...
        for (Entry<String, Object> e : new TreeMap(this.mStorage.getAll()).entrySet()) {
            String key = (String) e.getKey();
            if (key.startsWith(PREFIX)) {
                if (key.endsWith("TimeMillis")) {
                    Time time = new Time();
                    time.set(((Long) e.getValue()).longValue());
